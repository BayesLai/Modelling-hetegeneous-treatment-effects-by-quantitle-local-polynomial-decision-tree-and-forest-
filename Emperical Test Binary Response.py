...
with open("NSW-data.csv",'r')as myfile:
    ...
    for i in list(csvreader)[1:]:
        ...
        wage_gap.append(1 if float(i[9])>float(i[8]) else 0)
he_varlist=[education,age]
QLPRT_result=QLPRT(wage_gap,[treat,black,hispanic,married,nodegree],he_varlist,Response_type='binary',treat_var_index=1)
QLPRT_DHG= QLPRT_result.DHG
QLPRF_result=QLPRF(wage_gap,[treat,black,hispanic,married,nodegree],he_varlist,Response_type='binary',treat_var_index=1)
...
for j in range(len(he_varlist)):
    ...
    '''
    file_name = "Binary" + "-QLPRT-" + str(j + 1)+'.csv'
...
for j in range(len(he_varlist)):
    ...
    '''
    file_name = "Binary" + "-QLPRF-" + str(j + 1)+'.csv'
    with open(file_name, 'a')as myfile:
        csvwriter = writer(myfile)
